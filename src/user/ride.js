...
async function showDriverAssigned(driverId, state) {
  ...
  const conductorWindowHTML = `
    <div class="zippy-window">
      <div style="background:rgba(255,255,255,.03); border:1px solid rgba(48,209,88,0.2); border-radius:16px; padding:12px; box-shadow: 0 4px 20px rgba(0,0,0,0.2); height:145px; width:92%; margin:0 auto; box-sizing:border-box;">
        <div style="margin-bottom:6px;">
          <span style="color:rgba(255,255,255,.4); font-size:8px; display:block; text-transform:uppercase; letter-spacing:1px; margin-bottom:1px;">Tu Conductor</span>
          <span style="color:#fff; font-size:16px; font-weight:800; display:block;">${driverName}</span>
          <span style="color:#FFD700; font-size:11px; font-weight:700; display:block;">${driverRating}</span>
        </div>
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; height: 65px;">
